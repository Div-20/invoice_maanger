...
$collapse_toggle = 'toggle_id_string_demo5';
...
$clone_counter = isset($clone_counter) ? $clone_counter : 500;
@endphp
<div class="input-content">
    <div class="d-flex w-100 justify-content-between dynamic-form-heading">
        <a href="#{{$collapse_toggle}}" class="info-box-text text-muted dynamic-collapse-target" data-toggle="collapse">
            <i class="fa fa-caret-down mr-2" aria-hidden="true"></i><span class="">{{$dynamic_form_obj::$input_type_labels[$dynamic_form_obj::TYPE_NUMBER]}}</span>
        </a>
    ...
    <div id="{{$collapse_toggle}}" class="collapse-panel-box collapse dynamic-collapse-target">
        <input type="hidden" class="form-control change_input_index" name="form_data[{{$input_form_name}}][input_type]" value="{{$dynamic_form_obj::TYPE_NUMBER}}">
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control change_input_index"  value="{{$input_data['default'] ?? ''}}" name="form_data[{{$input_form_name}}][default]" placeholder="Enter Default">
        </div>
