...
@php
    $page_title = 'Building Block';
    $add_new_url = route('admin.building-block.create');
@endphp
@section('title')
    {{ $page_title }}
@endsection
                        ...
                        <tbody>
                            @foreach ($aRows as $key => $item)
                                <tr>
                                    ...
                                    <td>{{ isset($item->building->name)?$item->building->name:'' }}</td>
                                    <td>{{ isset($item->floor->name)?$item->floor->name:'' }}</td>
                                    <td>{{ $item->room_no}}</td>
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{ route('admin.building-block.edit', $item->id) }}" class="btn btn-primary  btn-xs" title="Edit"><i class="fa fa-edit"></i></a>
                                            <form action="{{ route('admin.building-block.destroy',$item->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
