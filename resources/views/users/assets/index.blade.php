...
@php
    $page_title = 'Assets';
    $add_new_url = route('user.assets.create');
...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            {{ Form::open(['url' => url('/assets'), 'method' => 'GET', 'id' => 'btfform']) }}
            <div class="row">
            ...
            <div class="card">
                ...
                <div class="float-left">
                    <a href="{{ $add_new_url }}" class="btn btn-success rounded-0" style="margin:10px;"><i
                            class="fa fa-plus"></i> Add</a>
                    <a href="{{ route('user.assets.import') }}" class="btn btn-success rounded-0"
                        style="margin:10px;"><i class="fa fa-upload"></i>Import</a>
                    ...
                    <table class="projects table table-bordered table-hover table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ (isset($item->building->code)?$item->building->code:'').'/'.(isset($item->asset_type->code)?$item->asset_type->code:'').'/'.$item->unique_id }}</td>
                                ...
                                <td>
                                    <a href="{{ route('user.assets.show', $item->id) }}"
                                        class="btn btn-primary btn-xs" title="View"><i class="fa fa-eye"></i></a>

                                    @if($item->status == 1)
                                    <a href="{{ route('user.assets.qrcode', $item->id) }}"
                                        class="btn btn-primary  btn-xs" title="QRCode"><i class="fa fa-qrcode"></i></a>
                                    @endif
                                    @if($item->status == 0)

                                    <form action="{{ route('user.assets.destroy', ['asset' => $item->id]) }}" method="POST" style="display: inline-block;">
                                        @csrf
                                        @method('DELETE')
                                ...
                                <table  class="projects table table-bordered table-hover table-striped show_details mb-0">
                                    <tr>
                                        ...
                                        <th colspan="2" class="border_bottom w-100">
                                            <strong>{{ $asset->unique_id ? (isset($asset->building->code)?$asset->building->code:'').'/'.(isset($asset->asset_type->code)?$asset->asset_type->code:'').'/'.$asset->unique_id : 'N/A' }}</strong></th>
                                    </tr>
                                    <tr class="pl-2">
                                        <th>Building:</th>
